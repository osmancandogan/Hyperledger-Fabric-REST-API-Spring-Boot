...
class TokenERC721Contract extends Contract {
    ...
    /// CHECK if iterator is working
    async getAllNFTs(ctx, owner) {
        // Check contract options are already set first to execute the function
        await this.CheckInitialized(ctx);
    ...
    async updateStatus(ctx, tokenId, status) {
        // Check contract options are already set first to execute the function
        await this.CheckInitialized(ctx);
    ...
    */
    async Mint(ctx, tokenId, temperature, status, timestamp) {
        // Check contract options are already set first to execute the function
        await this.CheckInitialized(ctx);
        ...
        const temperatureDouble = parseFloat(temperature);
        if (isNaN(temperatureDouble)) {
        ...
        if (isNaN(timestampInt)) {
            throw new Error(`The timestamp ${timestamp} is invalid. timestamp must be an integer`);
        }
